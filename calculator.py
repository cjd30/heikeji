# -*- coding: utf-8 -*-
"""
Created on Wed Oct 14 22:05:00 2020
...
def chakan(entry, argu):
    root = Tk()
    root.resizable(0, 0)
    text = Text(root, width=20, height=10, bd=4, bg='#FFFACD', font=('微软雅黑', 12))
    text.grid(padx=5, pady=5)
    if argu == 'help':
        root.title('帮助')
        text.insert(INSERT, '有手就会用！\n')
        text.insert(INSERT, '简单快捷有效！')
    elif argu == 'author':
        root.title('作者')
        text.insert(INSERT, '黑科技团队:陈钧迪  张佳宁\n')
        text.insert(INSERT, 'TIME:2020-10-15')
